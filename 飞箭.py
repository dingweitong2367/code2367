...
class ArrowGame:
    ...
    def draw_start_screen(self):
        """绘制开始界面"""
        title = self.big_font.render("一箭又一箭", True, BLUE)
        self.screen.blit(title, (WIDTH//2 - title.get_width()//2, HEIGHT//3))
        
        tip = self.font.render("点击箭头，让所有箭头飞出棋盘！", True, DARK_GRAY)
        self.screen.blit(tip, (WIDTH//2 - tip.get_width()//2, HEIGHT//2))
        
        # 开始按钮
        start_btn = pygame.Rect(WIDTH//2 - 100, HEIGHT//2 + 80, 200, 60)
        pygame.draw.rect(self.screen, GREEN, start_btn, border_radius=15)
        pygame.draw.rect(self.screen, BLACK, start_btn, 2, border_radius=15)
    ...
    def draw_end_screen(self, is_win):
        """绘制通关/失败结算界面"""
        if is_win:
            text = self.big_font.render("恭喜通关！", True, GREEN)
            self.screen.blit(text, (WIDTH//2 - text.get_width()//2, HEIGHT//3))
            tip = self.font.render("点击任意位置返回开始界面", True, DARK_GRAY)
            self.screen.blit(tip, (WIDTH//2 - tip.get_width()//2, HEIGHT//2))
        else:
            # 失败界面
            text = self.big_font.render("游戏失败", True, RED)
            self.screen.blit(text, (WIDTH//2 - text.get_width()//2, HEIGHT//4))

            # 重新开始本关按钮
            pygame.draw.rect(self.screen, BLUE, self.lose_restart_btn, border_radius=12)
            pygame.draw.rect(self.screen, BLACK, self.lose_restart_btn, 2, border_radius=12)
            btn1_text = self.font.render("重新开始本关", True, WHITE)
            self.screen.blit(btn1_text, (self.lose_restart_btn.centerx - btn1_text.get_width()//2,
                                         self.lose_restart_btn.centery - btn1_text.get_height()//2))

            # 返回主菜单按钮
            pygame.draw.rect(self.screen, GRAY, self.lose_back_btn, border_radius=12)
            pygame.draw.rect(self.screen, BLACK, self.lose_back_btn, 2, border_radius=12)
            btn2_text = self.font.render("返回主菜单", True, BLACK)
            self.screen.blit(btn2_text, (self.lose_back_btn.centerx - btn2_text.get_width()//2,
                                         self.lose_back_btn.centery - btn2_text.get_height()//2))
    ...
    def draw(self):
        ...
        if self.state == STATE_START:
            ...
            self.draw_end_screen(is_win=True)
        elif self.state == STATE_LOSE:
            self.draw_end_screen(is_win=False)
